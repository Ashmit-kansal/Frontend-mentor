...
function Header() {
  ...
  const {theme, subject, subjectImage} = useSelector((state) => state.quiz);
  const dispatch = useDispatch();

  return (
    // {/* header will remain fixed at the top of the page */}
        <header className="flex justify-between">
          {/* this div will show which subject is currently selected */}
          <div>
            <div>{subjectImage}</div>
            <p className={`text-preset4 ${theme === 'light' ? "text-blue-900": "text-white"}`}>{subject}</p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => dispatch(toggleTheme())} className={`p-1 w-9 h-5 rounded-2xl bg-purple-600 flex ${theme === 'light' ? "justify-start" : "justify-end"} items-center`}>
